...
function applyGravity(obj) {
    ...
    for (const gp of gravityPoints) {
        ...
        if (distance < CONFIG.gravityRadius) {
            ...
            if (!obj.beingSucked) {
                // ถ้ากำลังพุ่งลงมาแรงมาก (โยน) หรือมี velocity สูง ให้ข้าม gravity pull เพื่อไม่ให้เด้ง
                const hasFastVelocity = Math.abs(obj.vx) > 15 || obj.vy > 15;
                
                if (hasFastVelocity) {
                    // ไม่ดึงเลย - ให้มันพุ่งผ่านไปตามโมเมนตัม
                } else {
                    // ดึงปกติเมื่อความเร็วไม่สูงมาก (ตกลงมาช้าๆ)
    ...
    if (!state.isFrozen) {
        ...
        for (let i = state.objects.length - 1; i >= 0; i--) {
            ...
            if (!obj.dragging) {
                ...
                // Wall Collision
                // ปิด bounce ถ้าอยู่ใกล้ด้านล่าง (บริเวณหลุมดำ) เพื่อไม่ให้เด้งกลับ
                const nearBottom = obj.y > playArea.clientHeight * 0.5; // ลดจาก 0.6 เป็น 0.5
                
                if (!nearBottom) {
                    // ชนฝาเฉพาะตอนอยู่ด้านบน
                    if (obj.x <= 0) {
                        obj.x = 0;
                    ...
                    // บริเวณหลุมดำ - ลด velocity แนวนอนลงอย่างรวดเร็ว และไม่ bounce
                    obj.vx *= 0.85; // ลดความเร็วลงเร็วขึ้น
                    
                    if (obj.x <= 0) {
                        obj.x = 0;
                        obj.vx = 0; // หยุด bounce
                    } else if (obj.x >= playArea.clientWidth - obj.width) {
                        obj.x = playArea.clientWidth - obj.width;
                        obj.vx = 0; // หยุด bounce
                    }
